...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        printExample("Example 10");

        separator();

        OutOfBounds outOfBounds = new OutOfBounds();
        System.out.println(outOfBounds.getItemFromList(3));
        try {
            int index = (int)Math.random();
            System.out.println(index);
            System.out.println(outOfBounds.getItemFromList(index));
        } catch (IndexOutOfBoundsException e){
            System.out.println(e.getMessage() + " " + e.toString());
            System.out.println("index is out of bounds");
        } finally {
            System.out.println("catching works fine");
        }
        //uncomment before explaining

        //yet another source of possible Exceptions - let's scan some text!
        separator();
        printExample("SCANNER TEST");
        separator();
        System.out.println();
        MyScanner myScanner = new MyScanner();
        System.out.println(myScanner.findInteger("1 tekst test 2"));
